...
Connection.prototype.connect = function() {
  ...
  function cacheCleaner(self) {
    const now = Date.now()
    let totalCleaned = 0
    for (const [key, value] of Object.entries(self._curReq.fetchCache)) {
      if (now - value.t > 10000) {
        delete self._curReq.fetchCache[key]
        totalCleaned++
      }
    }
    if (totalCleaned > 0) console.log(`>>> cleaned ${totalCleaned} _curReq.fetchCache items`)
  }
  ...
  parser.on('body', function(stream, info) {
    ...
    if (msg === undefined) {
      ...
      const len = Object.keys(self._curReq.fetchCache).length
      if (len % 1000 === 0) {
        // console.log(`>>> emitting message ${len}`)
        cacheCleaner(self)
      }
